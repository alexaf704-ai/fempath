...
def render_result(data):
    ...
    # This week actions — rendered individually to avoid Streamlit HTML nesting issues
    st.markdown("""
    ...
    for i, a in enumerate(data['week_actions'], 1):
        title = str(a.get('title', '')).replace('<', '&lt;').replace('>', '&gt;')
        detail = str(a.get('detail', '')).replace('<', '&lt;').replace('>', '&gt;')
        cta    = str(a.get('cta',    '')).replace('<', '&lt;').replace('>', '&gt;')
        st.markdown(f"""
        <div style="display:flex;align-items:flex-start;gap:14px;
                    background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.09);
                    border-radius:12px;padding:16px;margin-bottom:10px;">
          <div style="min-width:28px;height:28px;background:#00C07F;color:#0d1b2a;
          ...
          <div style="flex:1;">
            ...
            <div style="font-size:12px;color:#8fa3b8;line-height:1.5;">{detail}</div>
          </div>
          <span style="font-size:11px;font-weight:700;color:#00C07F;
                       background:rgba(0,192,127,0.1);padding:4px 10px;
                       border-radius:20px;white-space:nowrap;flex-shrink:0;">{cta}</span>
        </div>
        """, unsafe_allow_html=True)
